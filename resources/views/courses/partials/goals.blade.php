        ...
        <ul class="divide-y rounded border bg-base-100 border-base-300" @dragover.prevent @drop="onDrop">
            @foreach ($course->goals()->orderBy('position')->get() as $goal)
                <li class="flex items-center gap-2 p-2 cursor-move hover:bg-gray-50" draggable="true"
                    @dragstart="onDragStart($event, {{ $goal->id }})" @dragover.prevent="onDragOver($event, $el)"
                    data-id="{{ $goal->id }}">
                    <span class="text-base-content/50">≡</span>
                    <span class="flex-1">{{ $goal->text }}</span>
                    <div class="flex gap-2">
                        <button class="btn btn-xs"
                            @click="openEdit({ id: {{ $goal->id }}, text: @js($goal->text) })">Editar</button>
                        <button class="btn btn-xs btn-error"
                            @click="openDelete({ id: {{ $goal->id }}, text: @js($goal->text) })">Eliminar</button>
                    </div>
                </li>
            @endforeach
            @if ($course->goals()->count() === 0)
                <li class="p-3 text-sm text-base-content/60">Aún no hay metas definidas.</li>
            @endif
        </ul>
        <div class="flex justify-end">
                ...
                <template x-if="modal.type==='create' || modal.type==='edit'">
                    <form :action="modal.action" method="POST" class="space-y-4 mt-3">
                        ...
                        <input name="text" class="input input-bordered w-full" :value="modal.payload.text || ''"
                            placeholder="Descripción de la meta" required>
