@if (isset($course))
    <div x-data="reqsManager()" x-init="init()" class="space-y-4">
        ...
        <ul class="divide-y rounded border bg-base-100 border-base-300" @dragover.prevent @drop="onDrop">
            @foreach ($course->requirements()->orderBy('position')->get() as $req)
                <li class="flex items-center gap-2 p-2 cursor-move hover:bg-gray-50" draggable="true"
                    @dragstart="onDragStart($event, {{ $req->id }})" @dragover.prevent="onDragOver($event, $el)"
                    data-id="{{ $req->id }}">
                    <span class="text-base-content/50">≡</span>
                    <span class="flex-1">{{ $req->text }}</span>
                    <div class="flex gap-2">
                        <button class="btn btn-xs"
                            @click="openEdit({ id: {{ $req->id }}, text: @js($req->text) })">Editar</button>
                        <button class="btn btn-xs btn-error"
                            @click="openDelete({ id: {{ $req->id }}, text: @js($req->text) })">Eliminar</button>
                    </div>
                </li>
            @endforeach
            @if ($course->requirements()->count() === 0)
                <li class="p-3 text-sm text-base-content/60">Aún no hay requisitos definidos.</li>
            @endif
        </ul>
        <div class="flex justify-end">
            ...
            <div class="modal-box">
                ...
                <template x-if="modal.type==='create' || modal.type==='edit'">
                    <form :action="modal.action" method="POST" class="space-y-4 mt-3">
                        @csrf
                        <template x-if="modal.type==='edit'"><input type="hidden" name="_method"
                                value="PUT"></template>
                        <input name="text" class="input input-bordered w-full" :value="modal.payload.text || ''"
                            placeholder="Descripción del requisito" required>
    ...
    <script>
        function reqsManager() {
            return {
                ...
                onDragStart(e, id) {
                    this.draggingId = id;
                    this.draggingEl = e.target.closest('[data-id]');
                ...
                onDrop(e) {
                    ...
                    this.$refs?.reorderForm?.scrollIntoView({
                        behavior: 'smooth',
                        block: 'nearest'
                    });
                },
                openCreate() {
                    this.modal = {
                        ...
                        action: @js(route('courses.requirements.store', $course)),
                        payload: {}
                    };
                    this.$nextTick(() => this.$refs.reqModal?.showModal());
                },
                openEdit(req) {
                    this.modal = {
                        open: true,
                        ...
                        action: @js(route('courses.requirements.update', [$course, 0])).replace('/0', '/' + req.id),
                        payload: req
                    };
                    this.$nextTick(() => this.$refs.reqModal?.showModal());
                },
                openDelete(req) {
                    this.modal = {
                        ...
                        action: @js(route('courses.requirements.destroy', [$course, 0])).replace('/0', '/' + req.id),
                        payload: req
                    };
                    this.$nextTick(() => this.$refs.reqModal?.showModal());
                },
