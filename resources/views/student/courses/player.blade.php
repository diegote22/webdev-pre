...
<x-app-layout>
                                    ...
                                    @foreach ($sec->lessons as $l)
                                        <li>
                                            <a href="{{ route('student.courses.lesson', [$course, $l]) }}"
                                                ...
                                                <span
                                                    class="truncate inline-block max-w-[140px] align-middle">{{ $l->title }}</span>
                                                @if ($l->duration)
                                                    <span
                                                        class="ml-1 text-[10px] opacity-70">{{ $l->duration }}m</span>
                                                @endif
                                            </a>
                                        </li>
                                    @endforeach
                ...
                <div class="bg-base-100 rounded shadow overflow-hidden">
                    <div class="aspect-video w-full bg-black flex items-center justify-center relative">
                        ...
                        @elseif ($lesson->video_type === 'external' && $lesson->video_url)
                            <video x-ref="player" class="w-full h-full" controls playsinline
                                @timeupdate="trackProgress($event)">
                    ...
                    <div class="p-4 space-y-3">
                        ...
                        <div class="tabs tabs-bordered">
                            <a class="tab tab-active" @click.prevent="activeTab='materials'"
                                :class="{ 'tab-active': activeTab==='materials' }">Material</a>
                            <a class="tab" @click.prevent="activeTab='info'"
                                :class="{ 'tab-active': activeTab==='info' }">Info</a>
                        </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="flex flex-wrap gap-2 mb-2">
                                            @foreach ($pdfs as $idx => $p)
                                                <button type="button" class="btn btn-xs"
                                                    @click="loadPdf('{{ Storage::url($p->path) }}')">{{ $p->name }}</button>
                                            @endforeach
                                        </div>
                                        <div class="border rounded relative" x-show="currentPdfUrl">
                                            ...
                                            <div class="flex items-center gap-2 p-2 text-xs">
                                                ...
                                                <span>Página <span x-text="pageNum"></span>/<span
                                                        x-text="pageCount"></span></span>
                                            </div>
                                        </div>
                                    </div>
                                @endif
                                            ...
                                            @foreach ($images as $img)
                                                <a href="{{ Storage::url($img->path) }}" target="_blank"
                                                    ...
                                                    <img src="{{ Storage::url($img->path) }}"
                                                        alt="{{ $img->name }}"
                                                        class="rounded border object-cover aspect-video group-hover:opacity-80" />
                                                    <span
                                                        class="block mt-1 text-[10px] truncate">{{ $img->name }}</span>
                                                </a>
                                            @endforeach
                                        </div>
                                    </div>
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js"
        integrity="sha512-h0QmkavRKu0kUQeFkC6qJbXA3ouzJXDo8wZfC4fVzz4YI8U/dKOppJ6n9LILSTNh8xmg7tKZYdnbHUiVDgE59w=="
    ...
    <script>
        function coursePlayer() {
            return {
                ...
                renderPage(num) {
                    ...
                    this.pdfDoc.getPage(num).then(page => {
                        ...
                        renderTask.promise.then(() => {
                            ...
                            if (this.queuePage !== null) {
                                const q = this.queuePage;
                                this.queuePage = null;
                                this.renderPage(q);
                            }
                        });
                ...
                nextPage() {
                    if (!this.pdfDoc || this.pageNum >= this.pageCount) return;
                    this.queueRenderPage(this.pageNum + 1);
                },
                trackProgress(e) {
                    const el = e.target;
                    ...
                    if (percent >= 90 && !this.sentComplete) {
                        this.sentComplete = true;
                        this.sendProgress(el.currentTime, true);
                    } else if (percent % 10 === 0) {
                        this.sendProgress(el.currentTime, false);
                    }
                },
